...
function Invoke-PgBackup($DbName, $BackupDir, $Period = "", [int]$StorePeriods = 0, $Srvr = $null, $Port = $null) {
    ...
    if ($Srvr -ne $null) {
        $PgArgs = $PgArgs + ' --host=' + $Srvr
    }

    if ($Port -ne $null) {
        $PgArgs = $PgArgs + ' --port=' + $Port
    }
